#!/usr/bin/env python3
"""Generated stdlib Gate A delivery; reuse the operator's unchanged V2 ZIP."""
import base64
import hashlib
import json
...
def _safe_path(name):
    if not isinstance(name, str) or not name or "\\" in name or "\x00" in name:
        raise ValueError("Unexpected Gate A payload path.")
...
def _decode_patch():
    ...
    if hashlib.sha256(raw).hexdigest() != PATCH_SHA256:
        raise ValueError("Gate A payload checksum mismatch.")
    patch = json.loads(raw)
    if set(patch) != {"schema_version", "base_commit", "source_commit", "files", "deleted_paths"}:
        raise ValueError("Unexpected Gate A payload fields.")
    if patch["schema_version"] != "1.0" or patch["base_commit"] != "00f07d7":
        raise ValueError("Unsupported Gate A patch baseline.")
...
def prepare_source(archive_path, destination):
    ...
    if destination.is_symlink() or (destination.exists() and (not destination.is_dir() or any(destination.iterdir()))):
        raise ValueError("Gate A extraction requires a new empty directory.")
    ...
    for name, (content, mode) in patched_files.items():
        ...
        target.chmod(mode)
    if not (destination / "scripts/launch_gate_a.py").is_file():
        raise ValueError("Gate A launcher is missing from committed source.")
    return commit
...
def main(argv=None):
    ...
    if not archive.is_file():
        ...
        return 1
    try:
        directory = Path(tempfile.mkdtemp(prefix="authority-delta-gate-a-"))
        commit = prepare_source(archive, directory)
        print(f"Authority Delta Gate A prepared. Source: {commit}", flush=True)
        ...
        return subprocess.run([sys.executable, str(directory / "scripts/launch_gate_a.py")], cwd=directory).returncode
