#!/usr/bin/env python3
"""Build a stdlib-only Gate A entrypoint from committed changes to the V2 base."""
import base64
import hashlib
import json
...
def build(archive_path, destination):
    ...
    for status, encoded_name in zip(entries[::2], entries[1::2]):
        ...
        if kind != b"blob" or mode not in (b"100644", b"100755"):
            raise ValueError("Gate A payload supports regular committed files only.")
    ...
    _git("cat-file", "-e", f"{commit}:scripts/launch_gate_a.py")
    template = _git("show", f"{commit}:scripts/gate_a_entrypoint.py").decode()
...
if __name__ == "__main__":
    print(build(ROOT.parent / "outputs/ReadinessOps_Authority_Delta_AWS_Verification_V2.zip",
                ROOT.parent / "outputs/Authority_Delta_Gate_A.py"))
