"""Bounded HTTP entrypoint for read-only Strands analysis on AgentCore Runtime."""
import json
import os
import subprocess
import sys
...
def perform(source):
    ...
    from authority_delta.analysis import EvidenceReader
    ...
    model=BoundedModel(model_id=model_id,region_name=region,temperature=0,max_tokens=3000,streaming=False,
    ...
    result.update(result='OBSERVED' if result.get('analysis_status')=='VALIDATED' else 'HOLD',input_hash=source['input_hash'],model_id=model_id,model_calls=calls,
