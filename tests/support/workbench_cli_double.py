...
action = args[:2]
BUILD_ID = "authority-delta-deploy:offline-workbench"
SCOPE = "AUTHENTICATED_REVIEW_WORKBENCH_NO_APPROVAL_OR_PUBLICATION"
text_identity = action == ["sts", "get-caller-identity"] and "--query" in args
...
if action != ["cognito-idp", "admin-set-user-password"]: assert value("--output") == ("text" if text_identity else "json")
...
if os.environ.get('AD_TEST_UPDATE')=='1' and mode!='WORKSPACE_CHANGED':
    OUTPUTS['WorkspaceUrl']='https://d3rn3hqm0ax5ux.cloudfront.net/'
if action == ["cognito-idp","admin-get-user"]:
    emit({"UserStatus":"FORCE_CHANGE_PASSWORD" if mode in ("PASSWORD_REQUIRED","PASSWORD_DENIED","PASSWORD_AMBIGUOUS","PASSWORD_POLICY") and not (state/"password-done").exists() else "CONFIRMED"})
if action == ["cognito-idp","admin-set-user-password"]:
    assert value('--output')=='json' and '--no-cli-auto-prompt' in args
    path=value('--cli-input-json').removeprefix('file://')
    with open(path) as stream:
        body=json.load(stream);stream.seek(0);assert body==json.load(stream)
    assert body=={"UserPoolId":"ap-northeast-1_test","Username":"okada","Password":"LocalPassword123","Permanent":True}
    if mode=='PASSWORD_DENIED':
        fail('An error occurred (AccessDeniedException) when calling the AdminSetUserPassword operation: '+body['Password'])
    if mode=='PASSWORD_POLICY' and not (state/'policy-rejected').exists():
        (state/'policy-rejected').write_text('once')
        fail('An error occurred (InvalidPasswordException) when calling the AdminSetUserPassword operation: '+body['Password'])
    (state/"password-done").write_text("confirmed")
...
if action == ["cloudformation", "describe-stacks"]:
    ...
    if name=="authority-delta-workbench":
        emit({"Stacks":[{"StackStatus":"CREATE_COMPLETE","Outputs":[{"OutputKey":k,"OutputValue":v} for k,v in OUTPUTS.items()]}]})
...
if action == ["s3api", "put-object"]:
    ...
    with zipfile.ZipFile(archive) as source:
        ...
        assert {"scripts/gate_b.py", "scripts/gate_a_state.py", "scripts/build_runtime_artifact.py",
            ...
            "buildspec.workbench.yml", "requirements-deploy.lock", "services/workbench/assets/app.js", "services/workbench/assets/manifest.json", "infra/workbench/template.json"} <= names
        ...
        assert b"scripts/deploy_workbench.py" in source.read("buildspec.workbench.yml")
        if os.environ.get('AD_TEST_UPDATE')=='1':
            assert 'packages/contracts/readinessops.schema.json' in names
            assert {'src/authority_delta/readiness.py','src/authority_delta/adapter_contract.py','src/authority_delta/adapters/vendor_payment_view.py'}<=names
            assert b'ReadinessOps' in source.read('services/workbench/assets/app.js')
...
if action == ["codebuild", "start-build"]:
    ...
    assert value("--buildspec-override") == "buildspec.workbench.yml"
    ...
    assert set(variables) == {"AD_REPORT_KEY", "AD_SOURCE_SHA256"}
    assert variables["AD_SOURCE_SHA256"] == source["hash"]
    expected_report = r"evidence/[0-9a-f]{32}/workbench-result\.json"
    assert re.fullmatch(expected_report, variables["AD_REPORT_KEY"])
...
if action == ["s3api", "get-object"]:
    ...
    report = {"scope": SCOPE, "build_id": BUILD_ID, "source_sha256": variables["AD_SOURCE_SHA256"],
        ...
        "resources":OUTPUTS, "gate_b":"UI_LOGIN_AND_REVIEW_PENDING",
    ...
    if mode == "FAIL":
        report['error']={'type':'ValueError','message':'Evidence binding changed'}
    if mode == "REPORT_MISMATCH": report["source_sha256"]="f"*64
