...
action = args[:2]
BUILD_ID = "authority-delta-deploy:offline-gate-a"
...
if action == ["logs", "get-log-events"]:
    ...
    emit({"events": [{"timestamp": 1, "message": "[Gate A] offline diagnostic retained"}], "nextForwardToken": "end"})
if action == ["s3api", "put-object"]:
    ...
    with zipfile.ZipFile(archive) as source:
        ...
        assert {"scripts/gate_a.py", "scripts/gate_a_state.py", "scripts/build_runtime_artifact.py",
            ...
            "buildspec.gate-a.yml", "requirements-deploy.lock"} <= names
        ...
        assert b"scripts/gate_a.py" in source.read("buildspec.gate-a.yml")
...
if action == ["codebuild", "start-build"]:
    ...
    assert value("--buildspec-override") == "buildspec.gate-a.yml"
    ...
    assert re.fullmatch(r"evidence/[0-9a-f]{32}/gate-a-result\.json", variables["AD_REPORT_KEY"])
...
if action == ["s3api", "get-object"]:
    ...
    report = {"scope": SCOPE, "build_id": BUILD_ID, "source_sha256": variables["AD_SOURCE_SHA256"],
        ...
        "gate_a": "NOT_PASSED" if mode == "FAIL" else "PASS",
        "cleanup": {"status": "UNCONFIRMED" if mode == "CLEANUP_UNCONFIRMED" else "PASS"},
        "gate_b": "NOT_RUN", "_offline_test": True}
    if partial:
