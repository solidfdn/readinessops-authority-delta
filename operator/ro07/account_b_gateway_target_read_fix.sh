...
if ! git diff --cached --quiet; then
  git -c user.name='ReadinessOps Recovery' \
      ...
      commit -m 'Permit exact Gateway target reads for Policy validation'
fi
...
for statement in response['PolicyDocument']['Statement']:
    ...
    if any(action in expected_validation for action in actions):
...
print('ACCOUNT_B_GATEWAY_TARGET_READ_FIX_PASS')
PY
