...
if ! git diff --cached --quiet; then
  git -c user.name='ReadinessOps Recovery' \
      ...
      commit -m 'Permit only resource-scoped AgentCore Policy management'
fi
...
for statement in response['PolicyDocument']['Statement']:
    ...
    if (statement.get('Effect') == 'Allow'
            and gateway in resources
            and any(action.startswith('bedrock-agentcore:Manage')
                    for action in actions)):
        matches.append(set(actions))
expected = [{'bedrock-agentcore:ManageResourceScopedPolicy'}]
if matches != expected or admin:
    raise SystemExit('ERROR: The Publisher resource-scoped Policy permissions do not match the minimum boundary.')
print('ACCOUNT_B_RESOURCE_SCOPED_POLICY_FIX_PASS')
PY
